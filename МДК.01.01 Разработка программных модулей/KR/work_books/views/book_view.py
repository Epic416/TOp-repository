...
class BookView:
    ...
    def input_book_data(self, existing_book: dict = None) -> dict:
        ...
        genre = input(
            f"Жанр: {f'({existing_book.get("genre")}) ' if existing_book and existing_book.get('genre') else ''}")
        ...
        if year_str.strip():
            ...
            result['year'] = self.controller.CURRENT_YEAR
    ...
    def search_books(self):
        ...
        if choice == '1':
            ...
            books = self.controller.search_book(query, 'title')
            self.show_books(books, f"Результаты поиска по названию: '{query}'")
        elif choice == '2':
            query = input("Введите автора: ")
            books = self.controller.search_book(query, 'author')
    ...
    def add_book_form(self):
        ...
        success, message = self.controller.add_book(
            ...
            year=data.get('year', self.controller.CURRENT_YEAR),
    ...
    def edit_book_form(self):
        ...
        book = self.controller.model.get_by_id(book_id)
        if not book:
    ...
    def delete_book_form(self):
        ...
        success, message = self.controller.delete_book(book_id)
        self.show_message(message, success)
    ...
    def toggle_availability_form(self):
        ...
        success, message = self.controller.toggle_book_available(book_id)
        self.show_message(message, success)
